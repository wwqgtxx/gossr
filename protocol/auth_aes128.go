...
func (a *authAES128) UdpPreEncrypt(plainData []byte) (outData []byte, err error) {
	if a.userKey == nil {
		params := strings.Split(a.Param, ":")
		a.userId = make([]byte, 4)
		if len(params) >= 2 {
			if userId, err := strconv.ParseUint(params[0], 10, 32); err != nil {
				rand.Read(a.userId)
			} else {
	...
	buf := append(plainData, a.userId...)
	h := a.hmac(a.userKey, buf)
	return append(buf, h[:4]...), nil
}

func (a *authAES128) UdpPostDecrypt(plainData []byte) ([]byte, int, error) {
	plainLength := len(plainData)
	readLength := plainLength - 4
	key := a.Key
	if plainLength > 4 {
		d := plainData[:readLength]
		h := a.hmac(key, d)[:4]
		r := plainData[readLength:]
		if h[0] != r[0] || h[1] != r[1] || h[2] != r[2] || h[3] != r[3] {
			return nil, 0, ssr.ErrAuthAES128HMACError
		}
		return d, readLength, nil
	} else {
